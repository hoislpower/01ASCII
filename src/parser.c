...
// Parse a device description source file into a device description object file.
// -----------------------------------------------------------------------------
	...
	if(keyword == PROGRAM_DATA || keyword == DATA)
	{
		if(bitOrderLength > MAX_BIT_ORDER_LENGTH)
		{
			fprintf(stderr, "ERROR: Length of program data bit "
				"order exceeds the maximum number of %lu.\r\n",
...
int	SequenceLoop(deviceData *device, int keyword)
{
	...
	if(getCurrentSymbol() == ASTERISK_SYM)
	{
		...
		// get the preDataBlockAddrBitOrder and the wordAddressBitOrder
		// out of the bit array datastructure 
